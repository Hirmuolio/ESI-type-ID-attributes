...
def call_was_succesful(esi_response, job, attempts):
	...
	#[200, 204, 304] = All OK
	#[404,  400, 422] = not found or user error. Call was succesful. Check for these elsewhere
	...
	if 'Warning' in esi_response.headers:
		print('Warning: ',esi_response.headers['Warning'], '. The script needs to be updated')
	
	if esi_response.status_code in [200, 204, 304, 400, 404, 422]:
		return True
	else:
...
def make_many_calls(urls, headers = '', calltype='get', job = 'make ESI call'):
	...
	#print(responses)
	#print(call_was_succesful(esi_response=responses[0], job=job, attempts=0))
	...
	while check_errors:
		...
		for index  in range(number_of_responses):
			try:
				if not call_was_succesful(esi_response=responses[index], job=job, attempts=0):
					check_errors = True
